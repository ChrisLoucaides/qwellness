...
class TestGetAdvisorMeetingsView(TestCase):
    def setUp(self):
        self.client = Client()
    ...
    def when_the_user_requests_to_view_their_meetings(self):
        response = self.client.get(reverse('advisor-meetings'), {'id': self.advisor.id})
        return response

    def when_a_user_requests_to_view_their_meetings_with_an_incorrect_http_post_method(self):
        response = self.client.post(reverse('advisor-meetings'))
        return response

    def when_the_user_with_an_invalid_id_requests_to_view_their_meetings(self):
        response = self.client.get(reverse('advisor-meetings'), {'id': 999})
        return response
